...
export const generateQuiz = async () => {
  ...
  `;
        ...
        const cleadedText = text.replace(/```(?:json)?\n?/g, "").trim();
        const quiz = JSON.parse(cleadedText);
...
export const saveQuizResults = async (questions, answers, score) => {
    const { userId } = await auth();
    if (!userId) throw new Error("Unauthorized");
    ...
    if (wrongAnswers.length > 0) {
        const wrongQuestionsText = wrongAnswers.map(wa =>
            `Question: "${wa.question}"\n
            Correct Answer: "${wa.answer}"\n
    ...
    try {
        ...
        console.log("Error saving quiz results", error.message);
